...
export class FastChecker {
  ...
  private async pollCycle(): Promise<void> {
    ...
    // Inject if there's anything. #510: await the real submit confirmation
    // (the deferred ENTER, not just the initial paste write) before ACKing —
    // otherwise a message can be marked delivered while it silently dies
    // unsubmitted in a PTY that's torn down moments later (session refresh,
    // context handoff, hard-restart).
    if (messageBlock) {
